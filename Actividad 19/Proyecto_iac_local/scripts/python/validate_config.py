...
def perform_complex_validations(config_data, file_path):
    ...
    if "database_connector" in file_path:
        conn = config_data.get("connectionString")
        if not conn:
            errors.append(f"[{file_path}] Falta 'connectionString' para database_connector.")
        elif not conn.startswith("postgresql://"):
            errors.append(f"[{file_path}] 'connectionString' inválido: {conn}")
